...
Three file types live in the invisible app-private folder:
- **`curiouskie-<childName>.json`** — one per child (profile + interest tags + session summaries + learned objects + the encrypted API key for legacy reasons).
- **`curiouskie-settings.json`** — the parent's global `appSettings` row (key, PIN hash, models, time limit, languages, sound/camera flags). Pulled on every sign-in, pushed after every settings change (`pushSettingsToDrive` helper in `ParentSettingsScreen`).
- **`curiouskie-debug.txt`** — the parent-controlled debug log (push from Settings, fetch on the other device from Settings).
...
- `ck_theme_mode` — `'light'` (default) or `'system'` (`lib/theme.ts`)
- `ck_debug_enabled` — debug mode flag (`lib/debugLog.ts`)
- `ck_log_<YYYY-MM-DD>` — daily debug-log buckets, auto-pruned after 7 days
- `ck_debug_last_drive_sync` — last debug log Drive upload timestamp
- `ck_chunk_reloaded_at` — `lazyRetry` sentinel (prevents reload loops)
- `ck_sfx_muted`, `ck_cam_migrated` — flags

`lib/profiles.ts#deleteProfile` is the single place that tears down per-child data (DB rows + the localStorage keys above). It does **NOT** wipe `appSettings` even on last-child delete — the parent's Gemini key, PIN, models and time limit are preserved so adding a new child is friction-free. It also fires `deleteChildFromDrive` so the child doesn't reappear in another device's restore chooser.
...
### Theme + Debug instrumentation
- **Light by default.** `index.css` sets `color-scheme: light` on `<html>`. The opt-in `html.theme-system` class allows the OS theme (toggled from Settings → App Theme). `applyTheme(getThemeMode())` runs in `main.tsx` *before* React mounts so there is no dark flash.
- **Debug Mode** (`lib/debugLog.ts` + `components/DebugOverlay.tsx`) is parent-gated. When ON: every navigation, errors, console warns, and explicit `logAction(...)` breadcrumbs persist to a per-day localStorage bucket (auto-pruned past 7 days). The floating 🐞 overlay appears only with debug on. Export (Web Share / download) and Drive upload/fetch (cross-device) live in Settings. Errors and console.* are always captured silently regardless of the flag.
...
- Mascot avatars are **custom SVG faces** drawn in `LeoMascot.tsx` (lion/owl/bunny), not platform emoji, so the smile renders the same everywhere.
...
- Screen-level `lazy()` imports are wrapped in `lazyRetry` (in `App.tsx`) so a stale chunk hash after a deploy auto-reloads the page once instead of leaving a blank screen.
